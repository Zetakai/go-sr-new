...
//     // Recreate the body reader so that we can decode it after printing
//     r.Body = ioutil.NopCloser(bytes.NewBuffer(body))

//     // Decode the body into youtubeURL struct
//     var youtubeURL YouTubeURL
//     err = json.NewDecoder(r.Body).Decode(&youtubeURL)
//     if err != nil || youtubeURL.Title == "" || youtubeURL.User == "" {
//         http.Error(w, "Invalid request payload", http.StatusBadRequest)
...
func addURL(w http.ResponseWriter, r *http.Request) {
    // Read and print the request body for debugging (if needed)
    ...
    // Reset the body so it can be used again
    r.Body = io.NopCloser(bytes.NewBuffer(body))

    // Declare an instance of YouTubeURL
    var youtubeURL YouTubeURL

    // Decode the JSON request body into youtubeURL struct
    err = json.NewDecoder(r.Body).Decode(&youtubeURL)
    if err != nil || youtubeURL.Title == "" || youtubeURL.User == "" {
        http.Error(w, "Invalid request payload", http.StatusBadRequest)
        return
    ...
    // Print to confirm the value of User
    fmt.Printf("User: %s\n", youtubeURL.User)

    // The User field is now set from the request body
    actualTitle, matchedURL, err := findClosestYouTubeMatch(youtubeURL.Title)
    if err != nil {
        http.Error(w, "Error finding YouTube match", http.StatusInternalServerError)
        return
